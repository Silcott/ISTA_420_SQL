...
--Step 2: 
select custid, count(orderid) from Sales.Orders group by custid;
--Step 3:
select custid, count(orderid) from Sales.Orders group by custid order by count(orderid);
--Step 4:
select custid, count(orderid) from Sales.Orders group by custid order by count(orderid) desc;
--Step 5:
select top (1) custid,count(orderid) from sales.orders group by custid order by count(orderid) desc;
--Step 6:
